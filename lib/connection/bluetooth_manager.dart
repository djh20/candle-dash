...
class BluetoothManager with ChangeNotifier {
  ...
  Future<void> connectToDevice(BluetoothDevice device) async {
    ...
    try {
      ...
      await disconnect();
      _reconnectTimer = Timer(const Duration(seconds: 1), reconnect);
    }
  }

  Future<void> reconnect() async {
    if (bondedDeviceId == null) return;
  ...
  void _updateConnectionState(BluetoothConnectionState state) {
    ...
    // _ensureValidConnectionState();
  }

  // Future<void> _ensureValidConnectionState() async {
  //   if (connectionState == BluetoothConnectionState.connected && (!isBonded || !enabled)) {
  //     await disconnect();
  //   }
  // }
}